...
public class Application extends MessageCracker implements quickfix.Application {
    private final OrderMatcher orderMatcher = new OrderMatcher();//模拟撮合
    ...
    private void processOrder(Order order) {
        if (orderMatcher.insert(order)) {//加入撮合买卖队列
            acceptOrder(order);//改变订单的状态为New
            ArrayList<Order> orders = new ArrayList<>();
            orderMatcher.match(order.getSymbol(), orders);//撮合业务
            while (orders.size() > 0) {
                fillOrder(orders.remove(0));//发送订单成交的状态（完全成交or部分成交）
            }
    ...
    private void acceptOrder(Order order) {
        updateOrder(order, OrdStatus.NEW);//改变订单的状态
    }
