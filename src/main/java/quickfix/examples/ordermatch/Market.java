...
public class Market {
    ...
    private final List<Order> bidOrders = new ArrayList<>();//买队列
    private final List<Order> askOrders = new ArrayList<>();//卖队列
...
* */
    ...
    private void match(Order bid, Order ask) {
        double price = ask.getType() == OrdType.LIMIT ? ask.getPrice() : bid.getPrice();//如果卖家是限价，成交价格使用卖方价格；卖方是市价则使用买家价格
        long quantity = bid.getOpenQuantity() >= ask.getOpenQuantity() ? ask.getOpenQuantity() : bid.getOpenQuantity();//成交数量是少的一方
        //设置下订单
    ...
    public boolean insert(Order order) {
        //依据订单的买卖方向，加入到对应的买卖队列：买队列bid降序；卖队列ask升序
    ...
    private boolean insert(Order order, boolean descending, List<Order> orders) {
        if (orders.size() == 0) {//队列为空直接插入
            orders.add(order);
        } else if (order.getType() == OrdType.MARKET) {//如果是市价交易则直接插入（不排序）
            orders.add(0, order);
        } else {//入股是限价则进行排序（bid降序：ask升序）
